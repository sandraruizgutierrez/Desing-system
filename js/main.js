                              ...
                              <div class="flex items-center justify-end gap-1">
                                ${metricStrip
                                  .map(([abbr, value, variableName, field]) => {
                                    ...
                                    return `
                                      <div class="flex items-center gap-1" style="padding: 5px 12px; background: #F8EAEB; border: 1px solid #F7DFE1; border-radius: 6px; line-height: 1;">
                                        <button type="button" ${variableName ? `data-type-copy-var="${variableName}"` : ""} class="text-[13px] font-semibold text-slate-500 hover:text-slate-900 leading-none" title="${variableName ? `Copiar ${variableName}` : 'Clic para copiar'}">${abbr}</button>
                                        <span class="font-mono text-[13px] font-semibold text-slate-700 cursor-pointer leading-none" ${variableName ? `data-type-copy-value="${variableName}"` : ""} title="${variableName ? `Copiar ${variableName}` : ''}">${value}</span>
                                      </div>
                                    `;
                                  })
                                  .join("")}
